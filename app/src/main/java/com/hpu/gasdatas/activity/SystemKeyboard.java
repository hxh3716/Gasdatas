package com.hpu.gasdatas.activity;
...
import com.hpu.gasdatas.activity.bean.KeyModel;
import com.hpu.gasdatas.activity.impl.SystemOnKeyboardActionListener;
import com.hpu.gasdatas.activity.keyboard.MyKeyboardView;
...
public class SystemKeyboard extends FrameLayout {
    ...
    private void randomkey() {
        ...
        for (int i = 0, size = keyList.size(); i < size; i++) {
            ...
            if (label != null && Detityutil.isNumeric(label.toString())) {
                newkeyList.add(key);
            }
        }
    ...
    public void setEditText(@NonNull EditText editText, boolean isOpenNativeKeyBoard) {
        ...
        if (isOpenNativeKeyBoard) {
            Detityutil.showKeyboard(editText);
            setVisibility(GONE);
        } else {
            setVisibility(VISIBLE);
            Detityutil.disableShowSoftInput(editText);
            Detityutil.hideKeyboard(editText.getContext());
        }
    }
